...
async function registerRoutes(app2) {
  ...
  app2.get("/api/dashboard/stats", isAuthenticated2, async (req, res, next) => {
    try {
      ...
      const previousPeriodDocs = documents2.filter(
        (doc) => doc.createdAt > twoMonthsAgo && doc.createdAt <= oneMonthAgo
      );
      ...
      const previousPeriodAnnouncements = announcements2.filter(
        (ann) => ann.createdAt > twoMonthsAgo && ann.createdAt <= oneMonthAgo
      );
  ...
  app2.get("/api/content/:id", isAuthenticated2, async (req, res, next) => {
    try {
      const [contentItem] = await db.select().from(contentItems).where(eq2(contentItems.id, parseInt(req.params.id)));
      if (!contentItem) {
